...
while a:
      ...
      if y == None :
            ...
            def jumia(j):
                  ...
                  print(j)
                  for item in y:
                        J = [item['sku'],item['name'],item['categories'],item['prices']['price'],item['image'],item['brand'],item['isBuyable'],item["url"],item['rating']["average"]]
                        cur.execute("INSERT INTO JUMIA(SKU,NAME,CATEGORIES,LASTPRICE,ImG,brand,active,URL,RATING ) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(J)) 
                        conn.commit()
                        c +=1
                  conn.close()
            jumia(y)   
      ...
      #                   for item in y:  
      #                         V = item['url']
      #                         cur.execute("INSERT INTO category (url) values (%s)",(V))
      #                         conn.commit()
      #                         c +=1
      #                   conn.close()
      #             category(y)        
...
#             for items in list:
#                   print(items)
#                   cur.execute("INSERT INTO Category (sweetName) values (%s)",(items))
#                   conn.commit()
#             conn.close()
# Category(y)
